...
public abstract class TimeChartExtension {
	...
	public TimeChartExtension(Context c, String title,String ylabel, String[] titles,
		...
		autoFitChart();
		mView = ChartFactory.getTimeChartView(c, mDataSet,
				mRenderer,"MMM yyyy");
	}
	...
	/**
	 * This is the ONLY public method for appending data to a plot!
	 * As is such, it relies on abstract methods, which the subclasses
	 * must implement to get the proper behavior
	 * @param date <code>long</code> representation of the date to be used for this datapoint
	 * @param values one (or more) data values to add to the plot. Depending on the subclass
	 * 		implementation, a certain number of values should be present
	 * @param autofit a <code>true</code> value will cause the min/max settings for the plot
	 * 		to be re-calculated. Do this infrequently whenever possible, to help performance
	 */
...
//			}
			double locMinX = series.getMinX(),
			locMinY = series.getMinY(),
			locMaxX = series.getMaxX(),
			locMaxY = series.getMaxY();
			if(locMinX < minX)
				minX = locMinX;
			if(locMinY < minY)
				minY = locMinY;
